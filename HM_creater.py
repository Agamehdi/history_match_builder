...
st.markdown("Bu interfeys material balansını (Material Balance) qoruyaraq real yataq üçün sintetik history match və qrafikləri simulyasiya edir. Soldakı paneldən bütün dizayn və mismatch parametrlərini tam fərdiləşdirə bilərsən.")

# ==========================================
# 1. SIDEBAR: FULLY CUSTOMIZABLE CONTROLS
# ==========================================
st.sidebar.header("🎨 Styling & Control Panel")

st.sidebar.subheader("Measured Data (Nöqtələr)")
meas_color = st.sidebar.color_picker("Measured Data Color", "#1f77b4")
meas_size = st.sidebar.slider("Measured Marker Size", 10, 100, 30)
meas_alpha = st.sidebar.slider("Measured Alpha (Şəffaflıq)", 0.1, 1.0, 0.8)

st.sidebar.subheader("Simulation Line (Xətt)")
...
st.sidebar.subheader("⚙️ Match & Physics Tweaks")
mismatch_intensity = st.sidebar.slider("History Mismatch Intensity (Xaos)", 0.0, 3.0, 1.0)
...
def generate_data(mismatch_mult, p_drop_factor):
    ...
    wc_meas = wc_sim + np.random.normal(0, 3 * mismatch_mult, len(time_months))
    wc_meas = np.clip(wc_meas, 0, 100)
    ...
    # Reservoir Pressure (Material Balance)
    ...
    pressure_meas[40:50] -= 120 * mismatch_mult  # Özüm qəsdən pozduğum yer
...
# ==========================================
# 3. MATPLOTLIB PLOTTING ENGINE
# ==========================================
fig, axes = plt.subplots(3, 3, figsize=(16, 12), dpi=150)
axes = axes.flatten()

def draw_plot(ax, x, meas, sim, title, ylabel, is_special=False):
    if not is_special:
...
draw_plot(axes[6], t, data["pressure"][0], data["pressure"][1], "Reservoir Pressure (Material Bal.)", "Pressure (psi)")
